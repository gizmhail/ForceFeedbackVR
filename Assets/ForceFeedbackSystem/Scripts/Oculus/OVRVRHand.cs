...
namespace ForceFeedbackSystem
{
    public class OVRVRHand : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            copyHand.transform.position = grabbedObject.forceFeedbackSystem.vrWorldObject.transform.TransformPoint(grabbedObject.grabSpotLocalPosition);
            copyHand.transform.rotation = grabbedObject.forceFeedbackSystem.vrWorldObject.transform.rotation * grabbedObject.grabSpotLocalRotation;
            Debug.Log("k:" + grabbedObject.grabSpotLocalPosition);
